        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item, index) in addressListFilter" :class="{'checked': checkIndex === index}" @click="checkIndex = index" :key="index">
                    <dl>
                      <dt>{{item.userName}}</dt>
                      <dd class="address">{{item.streetName}}</dd>
                      <dd class="tel">{{item.tel}}</dd>
                    </dl>
                    <div class="addr-opration addr-del" @click="showModal(item.addressId)">
              ...
              <div v-show="addressList.length > addressListFilter.length" class="shipping-addr-more" @click="expand">
...
<script>
  ...
  export default{
    ...
    computed: {
      ...
      selectAddrId () {
        return this.addressList.length > 0 ? this.addressList[this.checkIndex].addressId : null
      }
    },
    methods: {
      getAddressList () {
        axios.get('/users/addressList').then(res => {
          if (res.data.status === 0) {
            ...
            let currentIndex = this.addressList.findIndex(item => {
              return item.isDefault
            })

            if (currentIndex > 0) {
              let list = this.addressList;
              [list[currentIndex], list[0]] = [list[0], list[currentIndex]]
            }
